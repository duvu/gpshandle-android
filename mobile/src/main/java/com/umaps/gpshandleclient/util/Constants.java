package com.umaps.gpshandleclient.util;

/**
 * Created by beou on 29/10/2014.
 */
public class Constants {

    public static final String KEY_AUTH             = "authentication";
    public static final String KEY_ACCOUNT          = "account";
    public static final String KEY_USER             = "user";
    public static final String KEY_PASSWORD         = "password";

    public static final String KEY_REQUEST          = "requests";
    public static final String KEY_COMMAND          = "command";
    public static final String KEY_LOCALE           = "locale";
    public static final String KEY_PARAMS           = "params";

    public static final String CMD_GET_GROUPS       = "getGroups";
    public static final String CMD_GET_USER_ACL     = "getUserAcl";

    public static final String CMD_GET_MAP_FLEET    = "getMapFleet";

    public static final String FLD_groupID          = "groupID";
    public static final String FLD_status           = "status";
    public static final String FLD_inclZones        = "inclZones";
    public static final String FLD_inclDebug        = "inclDebug";
    public static final String FLD_inclPOI          = "inclPOI";


    public static final String FLD_inclTime         = "inclTime";
    public static final String KEY_status           = "status";

    public static final String KEY_result           = "results";
    public static final String KEY_fields           = "fields";

    public static final String KEY_dfields          = "dfields";
    public static final String fld_groupid          = "groupid";
    public static final String fld_deviceid         = "deviceid";
    public static final String fld_description      = "description";
    public static final String fld_devicesingroup   = "devicesingroup";


    public static final String fld_countdevices     = "countdevices";


    public static final String EXTRA_ACCOUNTID      = "extra_accountid";
    public static final String EXTRA_USERID         = "extra_userid";
    public static final String EXTRA_PASSWD        = "extra_passwd";
    //sharedPreference store
    public static final String SHR_ACCOUNT          = "sharedAccount";
    public static final String SHR_USER             = "sharedUser";
    public static final String SHR_PASSWRD          = "sharedPassword";



    //--------------------------------------------------------------------------------------------//
    public static final String GROUP_ALL            = "all";
}
